...
void client_getmsg(void *ctx, char *topic, char *message, int msglen, char *replyto) {
	solard_client_t *c = ctx;
	solard_message_t *msg;

	/* XXX Dont modify MQTT's message - make a COPY first! */
//	message[msglen] = 0;
//	dprintf(4,"topic: %s, message(%d): %s\n", topic, msglen, message);
...
//			{ s->section_name, "id", "Client ID", DATA_TYPE_STRING,&s->name,sizeof(s->name), id },
	...
	sprintf(mqtt_info,"%s/%s",SOLARD_TOPIC_ROOT,s->mqtt_config.clientid);
	mqtt_sub(s->m,mqtt_info);
