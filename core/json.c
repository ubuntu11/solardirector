...
int json_iter(char *name, json_value_t *v, json_ifunc_t *func, void *ctx) {
        ...
        case JSONObject:
                o = v->value.object;
                dprintf(1,"count: %d\n", o->count);
                for(i=0; i < o->count; i++) if (json_iter(o->names[i],o->values[i],func,ctx)) return 1;
                break;
        case JSONArray:
                a = v->value.array;
                dprintf(1,"count: %d\n", a->count);
                for(i=0; i < a->count; i++) if (json_iter(0,a->items[i],func,ctx)) return 1;
                break;
	default:
		{
			char value[256];
			json_conv_value(DATA_TYPE_STRING,value,sizeof(value)-1,v);
