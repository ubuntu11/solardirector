...
SHARED_FUNCTION int GetDeviceName( DWORD DevHandle, char * DestBuffer, int len)
{
   ...
   int res;
	...
	dprintf(1,"DevHandle: %d, DestBuffer: %p, len: %d\n", DevHandle, DestBuffer, len);
                                        ...
                                        BYTE bChanIndex)
{
   ...
   int ii,res;
	...
	dprintf(1,"pdDevHandle: %d, pdChanHandles: %p, dMaxHandleCount: %d, wChanType: %x, bChanIndex: %d\n",
		pdDevHandle, pdChanHandles, dMaxHandleCount, wChanType, bChanIndex);
...
DWORD FindChannelName(DWORD devh, char * ChanName)
{
   TChannel * chan;
   TNetDevice * dev;
   int res;
...
**************************************************************************/
SHARED_FUNCTION int GetChannelName( DWORD dChanHandle, char * ChanName, DWORD ChanNameMaxBuf)
{
   TChannel * chan;
   int res;
...
//      iRes = yasdiInitialize(iniFile, pDriverNum);
      ...
      checkValueRangeSettingChannel = TRepository_GetElementInt( "Misc.checkValueRange", 1 /* true*/ );
