...
void _outpower(char *label, float l1, float l2, float l3) {
	char str[128];
...
float batcurmin, batcurmax;

void dbat(si_session_t *s) {
	if (outfmt) dfloat("Battery Current","%3.2f",s->data.battery_current);
	else {
		...
		fprintf(outfp,"%-25s %3.2f (min: %3.2f, max: %3.2f)\n","Battery Current",s->data.battery_current,
			batcurmin, batcurmax);
	}
}
...
void display_data(si_session_t *s,int mon_flag) {
	...
	dfloat("AC2 Frequency","%2.2f",s->data.ac2_frequency);
	dfloat("AC2 Current","%2.2f",s->data.ac2_current);
	dfloat("Battery Voltage","%3.2f",s->data.battery_voltage);
	if (mon_flag) dbat(s);
	else dfloat("Battery Current","%3.2f",s->data.battery_current);
...
#define NIDS (sizeof(ids)/sizeof(int))
	...
	dfloat("TotLodPwr","%3.2f",s->data.TotLodPwr);
	dfloat("PVPwrAt","%3.2f",s->data.PVPwrAt);
	dfloat("GdCsmpPwrAt","%3.2f",s->data.GdCsmpPwrAt);
	dfloat("GdFeedPwr","%3.2f",s->data.GdFeedPwr);
#endif
}
