...
#define SET(m,b) { s->bits.m = ((bits & b) != 0); dprintf(5,"bits.%s: %d\n",#m,s->bits.m); }
	...
	if (s->bits.GdOn != s->grid_state) {
		s->grid_state = s->bits.GdOn;
		dprintf(0,"AC2 %s\n",(s->grid_state ? "connected" : "disconnected"));
	}
	if (s->bits.GnRn != s->gen_state) {
		s->gen_state = s->bits.GnRn;
		dprintf(0,"Gen %s\n",(s->gen_state ? "running" : "stopped"));
	}

	/* 0x308 TotLodPwr */
	if (s->can_read(s,0x308,data,sizeof(data))) return 1;
	inv->load_watts =  (si_getshort(&data[0]) * 100.0);
