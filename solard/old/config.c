...
int get_mqtt_conf(solard_config_t *conf) {
...
int read_config(solard_config_t *conf) {
		...
		{ "solard", "database", "Database Connection", DATA_TYPE_STRING, &conf->db_name, sizeof(conf->db_name), "" },
		/* Forced values */
...
int reconfig(solard_config_t *conf) {
...
solard_config_t *get_config(char *filename) {
	solard_config_t *conf;

	conf = calloc(1,sizeof(solard_config_t));
	if (!conf) {
		perror("malloc solard_config_t");
		return 0;
	}
	conf->modules = list_create();
	conf->packs = list_create();
