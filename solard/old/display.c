...
void display(struct solard_config *conf) {
#if 0
...
#define NSUMM 4
#define NBSUM 2
	float temps[PACK_MAX_TEMPS][SOLARD_MAX_PACKS];
	float values[PACK_MAX_CELLS][SOLARD_MAX_PACKS];
	float summ[NSUMM][SOLARD_MAX_PACKS];
	float bsum[NBSUM][SOLARD_MAX_PACKS];
#endif
	...
	solard_pack_t *pp;
	char format[16];
	uint8_t cdbstat[SOLARD_MAX_PACKS];
	...
	while((pp = list_get_next(conf->packs)) != 0) {
		if (!solard_check_state(pp,SOLARD_PACK_STATE_UPDATED)) continue;
	...
	list_reset(conf->packs);
	while((pp = list_get_next(conf->packs)) != 0) {
		if (!solard_check_state(pp,SOLARD_PACK_STATE_UPDATED)) {
...
//				dprintf(1,"values[%d][%d]: %.3f\n", y, x, values[y][x]);
			...
			cdbstat[x] = 0;
			if (solard_check_state(pp,SOLARD_PACK_STATE_CHARGING)) cdbstat[x] |= SOLARD_PACK_STATE_CHARGING;
			if (solard_check_state(pp,SOLARD_PACK_STATE_DISCHARGING)) cdbstat[x] |= SOLARD_PACK_STATE_DISCHARGING;
			if (solard_check_state(pp,SOLARD_PACK_STATE_BALANCING)) cdbstat[x] |= SOLARD_PACK_STATE_BALANCING;
		}
	...
	if (inv) {
		...
		if (solard_check_state(inv,SOLARD_INVERTER_STATE_UPDATED)) {
			if (solard_check_state(inv,SOLARD_INVERTER_STATE_RUNNING))
				p = "Running";
			else
				p = "Off";
		} else {
...
//		dprintf(0,"\nInverter: None\n");
	...
	while((pp = list_get_next(conf->packs)) != 0) {
		if (solard_check_state(pp,SOLARD_PACK_STATE_UPDATED)) {
			str[0] = solard_check_state(pp,SOLARD_PACK_STATE_CHARGING) ? '*' : ' ';
			str[1] = solard_check_state(pp,SOLARD_PACK_STATE_DISCHARGING) ? '*' : ' ';
			str[2] = solard_check_state(pp,SOLARD_PACK_STATE_BALANCING) ? '*' : ' ';
		} else {
			str[0] = str[1] = str[2] = ' ';
		}
		str[3] = 0;
