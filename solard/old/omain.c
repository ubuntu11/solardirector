...
static sdagent_config_t *init(int argc, char **argv) {
	...
	char pack_type[SOLARD_MODULE_NAME_LEN+1],pack_transport[SOLARD_MODULE_NAME_LEN+1],pack_target[SOLARD_TARGET_LEN+1];
	sigset_t set;
	...
	log_open("solard",logfile,logopts);
...
//					do_shutdown(conf);
			...
			if (solard_check_state(pp,SOLARD_PACK_STATE_DISCHARGING)) pack_capacity_total += pp->capacity;
			packs_reported++;
		}
		...
		if (npacks) {
			...
			dprintf(2,"user_capacity: %2.2f, DC: %d, min_cap: %2.2f, pack_count: %d\n",conf->user_capacity,solard_check_cap(conf,SOLARD_DISCHARGE_CONTROL), min_cap, list_count(conf->packs));
			...
			} else if (solard_check_cap(conf,SOLARD_DISCHARGE_CONTROL)) {
