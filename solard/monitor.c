...
//		dprintf(1,"info->data: %p\n", info->data);
		...
		if (strcmp(info->role,SOLARD_ROLE_BATTERY) == 0) {
			...
			inv = info->data;
			if (!solard_check_state(inv,SOLARD_INVERTER_STATE_UPDATED)) continue;
			name = inv->name;
		...
		if (conf->agent_notify > 0) {
			if (diff >= conf->agent_notify) {
				dprintf(5,"notify_gone state: %d\n", solard_check_state(info,AGENTINFO_NOTIFY_GONE));
				if (!solard_check_state(info,AGENTINFO_NOTIFY_GONE)) {
					sprintf(reason,"%s %s has not reported for %d seconds",info->role,name,diff);
					list_add(notify_list,reason,strlen(reason)+1);
					solard_set_state(info,AGENTINFO_NOTIFY_GONE);
				}
				continue;
			} else {
				solard_clear_state(info,AGENTINFO_NOTIFY_GONE);
			}
		...
		if (bp) {
			...
			if (bp->voltage < 1.0 || bp->voltage > 100.0) {
				if (!solard_check_state(info,AGENTINFO_NOTIFY_VOLT)) {
					sprintf(reason,"Battery %s has an invalid voltage: %f",bp->name,bp->voltage);
					list_add(notify_list,reason,strlen(reason)+1);
					solard_set_state(info,AGENTINFO_NOTIFY_VOLT);
				}
			} else {
			...
			if (inv->soc <= 35.0) {
				...
				solard_clear_state(info,AGENTINFO_NOTIFY_VOLT);
			}
		}
	}
	if (count) {
	...
	while((info = list_get_next(conf->agents)) != 0) {
		...
		for(i=0; i < bp->ntemps; i++) {
			...
			if (bp->temps[i] <= 5 || bp->temps[i] >= 30) {
				if (!solard_check_state(info,AGENTINFO_NOTIFY_TEMP)) {
					sprintf(reason,"Battery %s %s temp: %.1f",bp->name,(bp->temps[i] < 5 ? "low" : "high"),bp->temps[i]);
					list_add(notify_list,reason,strlen(reason)+1);
					solard_set_state(info,AGENTINFO_NOTIFY_TEMP);
				}
				break;
			} else {
				solard_clear_state(info,AGENTINFO_NOTIFY_TEMP);
			}
		}
	}
