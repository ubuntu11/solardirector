...
static int si_init(sdagent_config_t *conf) {
#if 0
...
static void *si_new(sdagent_config_t *conf, ...) {
	si_session_t *s;
	va_list ap;
	...
	va_start(ap,conf);
	inv = va_arg(ap,solard_inverter_t *);
...
void si_grid_control(void *handle, int value) {
	...
	b = 0;
	if (solard_check_state(s->conf,SOLARD_GEN_ENABLE)) {
		b |= SI_GEN_ENABLE;
		b |= SI_CAN_SWITCH;
	}
	if (solard_check_state(s->conf,SOLARD_GRID_ENABLE)) {
		b |= SI_GRID_ENABLE;
		b |= SI_CAN_SWITCH;
	}
...
//	si_session_t *s = handle;
	...
	dprintf(1,"op: %d\n", op);
	switch(op) {
	case SOLARD_INVERTER_GRID_CONTROL:
...
solard_module_t si_module = {
	SOLARD_MODTYPE_INVERTER,
	"si",
	SOLARD_INVERTER_GRID_CONTROL  | SOLARD_INVERTER_GEN_CONTROL | SOLARD_INVERTER_POWER_CONTROL,
	si_init,
	si_new,
	si_open,
	si_read,
