...
SHARED_FUNCTION int yasdiInitialize(smanet_session_t *s) {
    ...
    */
   {
      ...
      TRepository_GetElementStr( "Misc.DebugOutput","", DebugFile, sizeof(DebugFile)-1 );
	dprintf(1,"DebugFile: %s\n", DebugFile);
...
#endif
	if (debug > 3) os_setDebugOutputHandle(stdout);
   
   YASDI_DEBUG(( VERBOSE_MESSAGE, "YASDI Library V" LIB_YASDI_VERSION_FULL " (%s)\n" SMA_COPYRIGHT "\n"
