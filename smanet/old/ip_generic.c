...
static int ip_dBytesReadTotal = 0;
static int (*ip_RegisterDevice) (TDevice * newdev);
static TOnDriverEvent ip_SendEventCallback;
...
static WORD Address2Port(char * pdot, WORD defaultPort);
static DWORD Address2Ip(char * pdot);
static DWORD inetAddr2DriverDeviceHandle(struct sockaddr_in * addr);
static void printIPAddress(struct sockaddr_in * addr, char * buffer );
...
BOOL ip_Open(TDevice *dev)
{
   ...
   */
   if (bind(me->fd, (struct sockaddr *) &me->ClientAddr, sizeof(me->ClientAddr)) == SOCKET_ERROR) {
              ...
              DWORD * DeviceHandle)
{
   ...
   YASDI_DEBUG((VERBOSE_HWL, "IP::Read(): read %d bytes from peer %s\n",
                             ...
                             ip_buffer ));
   ip_dBytesReadTotal += me->dBytesinRecvBuffer;
   ...
   {
      {
         ...
         ip_SendEventCallback( NULL, &event);
   ...
   Changes       : Author, Date, Version, Reason
                   ********************************************************
                   ...
                   Pruessing, 23.06.2001, 1.1, Uebergabe von Repository und
                                               ...
                                               Parameter
**************************************************************************/
//int SHARED_FUNCTION InitYasdiModule(void *RegFuncPtr, TOnDriverEvent eventCallback)
int SHARED_FUNCTION ip_init(void *RegFuncPtr, TOnDriverEvent eventCallback)
{
   ...
   ip_RegisterDevice = RegFuncPtr;
   ip_SendEventCallback = eventCallback;
...
**************************************************************************/
#if 0
staitc void SHARED_FUNCTION CleanupYasdiModule(void)
{
