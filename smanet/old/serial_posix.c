...
void serial_writeAsync(TDevice * dev, struct TNetPacket * frame, DWORD DriverDeviceHandle, TDriverSendFlags flags);
int (*serial_RegisterDevice)(TDevice * newdev);

/**************************************************************************
***** Global Variables  ***************************************************
***************************************************************************/

static int serial_dBytesReadTotal = 0;
                                  ...
                                  DWORD * DriverDevHandle)
{
   ...
   if ( dev->DeviceState == DS_ONLINE )
   {
      ...
      if (BytesRead < 0)
      {
	   ...
	   {
         serial_dBytesReadTotal += BytesRead;   
...
TDevice * serial_create(DWORD dUnit)
{
   ...
   */
   ...
   if (interface && priv)
   {
      ...
      */
      (*serial_RegisterDevice)( interface );
   }
...
int serial_init( void * RegFuncPtr, TOnDriverEvent eventCallback )
{
   ...
   serial_RegisterDevice = RegFuncPtr;
...
**************************************************************************/
SHARED_FUNCTION void serial_writeAsync(TDevice * dev, struct TNetPacket * frame, DWORD DriverDeviceHandle, TDriverSendFlags flags)
{
   CREATE_VAR_THIS(dev,struct TSerialPosixPriv *);
