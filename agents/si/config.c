...
static void getconf(si_session_t *s) {
	cfg_proctab_t mytab[] = {
		...
		{ s->ap->section_name, "charge_min_amps", 0, DATA_TYPE_FLOAT, &s->charge_min_amps, 0, "-1" },
		{ s->ap->section_name, "charge_creep", "Increase charge voltage in order to maintain charge amps", DATA_TYPE_BOOL, &s->charge_creep, 0, "yes" },
		{ s->ap->section_name, "sim_step", 0, DATA_TYPE_FLOAT, &s->sim_step, 0, ".1" },
