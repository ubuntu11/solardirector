...
//	printf("==> pub_main\n");
	...
	status = addstat(status,si.feeding,"feed");
	status = addstat(status,(si.input_source != CURRENT_SOURCE_NONE && si.dynfeed && si.feeding && data.GdOn && si.charge_mode),"dynfeed");
	//printf("status: %s\n", status);
	fields.push("status");
...
//	printf("j: %s\n", j);
	...
	mqtt.pub(SOLARD_TOPIC_ROOT+"/Agents/"+si.agent.name+"/"+SOLARD_FUNC_DATA,j,0);
	if (influx.connected) influx.write("si",mydata);
}
