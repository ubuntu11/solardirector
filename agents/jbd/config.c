...
static json_descriptor_t control_params[] = {
...
#define NCONTROL (sizeof(control_params)/sizeof(json_descriptor_t))
...
static struct parmdir {
	...
	{ "Controls", control_params, NCONTROL },
};
...
static int jbd_get_config(jbd_session_t *s, struct jbd_params *pp, json_descriptor_t *dp, uint8_t *data, int len) {
	char topic[200];
	char str[64],temp[72];
	uint16_t val;
	float fval;
...
static int jbd_set_config(jbd_session_t *s, uint8_t *data, struct jbd_params *pp, json_descriptor_t *dp, char *value) {
	char *p;
