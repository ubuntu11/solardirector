...
global logfile
global debug;
def dprintf(level, text, *args):
	if debug >= level:
		print(time.strftime("[%Y-%m-%d %H:%M:%S]", time.localtime()), text % args);
...
def mqtt_send(broker,clientid,topic,mq_user,mq_pass,sensors):
	...
	for sen in sensors:
		...
		if sen.value:
			...
			j[sen.name] = 0;
	dprintf(1,"grid_power: %d, last_val: %d", j["grid_power"], last_val);
	if j["grid_power"] == last_val:
		return;
	dprintf(1,json.dumps(j));
	dprintf(0,"%d",j["grid_power"]);
        # Test mqtt
	dprintf(1,"broker: " + broker + ", clientid: " + clientid + ", topic: " + topic)
...
#	logging.basicConfig(stream=sys.stdout, level=logging.DEBUG)
	...
	dprintf(1,"configfile: %s", args.config);
	config = ConfigParser()
#	myfile = Path(args.configfile)
	...
	while True:
		...
		diff = end-start
		dprintf(2,"diff: %d", diff);
		if interval > diff:
			dprintf(1,"sleeping for: %d...",(interval-diff)+1);
...
if __name__ == "__main__":
	...
	parser.add_argument("-i", "--interval", help="Wait time before read", type=int, default=10)
	args = parser.parse_args()
	global debug
	debug = args.debug;

	if args.log:
		dprintf(1,"logfile: %s",args.log);
