...
int sb_load_objects(sb_session_t *s) {
	json_object_t *o;
...
//				{ "Prio",DATA_TYPE_INT,&newobj->prio,0,0 },
				...
				{ "Scale",DATA_TYPE_FLOAT,&newobj->mult,0,0 },
...
int sb_get_objects(sb_session_t *s) {
	...
	if (sb_request(s,"data/ObjectMetadata_Istl.json",0)) return 1;
	if (!s->data) return 1;
...
//	s->data = json_parse_file(filename);
//	if (!s->data) return 1;
...
//			{ "WriteLevel",DATA_TYPE_INT,&newobj->wrlevel,0,0 },
	...
	str = json_dumps(json_object_get_value(ro),1);
